...
{
    public class ExchangeWithProfitReturnCommissionExcludedTests : BaseAlmusherTest
    {
        public ExchangeWithProfitReturnCommissionExcludedTests(ITestOutputHelper output, AlmuhserFixture fixture)
            : base(output, fixture) { }

        [Fact]
        public async Task ExchangeWithProfitReturnCommissionExcluded_ShouldSucceed()
        {
            Output.WriteLine("\n💱 اختبار: صرف LYD → USD مع ربح وعمولة غير مضمنة وإرجاع (Profit + Return + Commission Excluded)");
            Output.WriteLine("═══════════════════════════════════════════════════════════");
            ...
            // ================================================================
            decimal buyUsd = 100m;
            decimal sellLyd = 800m;
            decimal commissionAmount = 10m;
            decimal profitRatio = 0.02m;
            decimal returnAmount = 10m;
            ...
            decimal expectedProfitInLyd = (profitRatio + expectedBaseRate) * (buyUsd + commissionAmount) - sellLyd - expectedCommissionInLyd;  // 101

            // 2.5 المبلغ النهائي بالـ LYD (مع العمولة والربح)
            decimal expectedSellFinalAmount = sellLyd + expectedCommissionInLyd + expectedProfitInLyd;  // 80901

            // 2.6 المبلغ النهائي بالـ USD (مع العمولة)
            decimal expectedBuyFinalAmount = buyUsd + commissionAmount;  // 10100

            // 2.7 المبلغ النهائي بالـ USD (مع العمولة والإرجاع)
            decimal expectedBuyAmountWithReturn = buyUsd + commissionAmount + returnAmount;  // 11100

            // 2.8 سعر الصرف النهائي (مع الإرجاع)
            decimal expectedConversionRate = expectedSellFinalAmount / expectedBuyAmountWithReturn;  // 7.2883783784
            ...
            Output.WriteLine($"   💰 المبلغ النهائي بالـ USD (مع الإرجاع): {expectedBuyAmountWithReturn:F2}");
            ...
            // ================================================================
            // 7. بناء طلب الصرف مع الربح والعمولة غير المضمنة والإرجاع
            // ================================================================
            Output.WriteLine("\n💱 بناء طلب الصرف مع الربح والعمولة غير المضمنة والإرجاع...");
            ...
            var request = Almusher.CreateExchangeRequest(
                ...
                lydRate: lydRate,
                detailedStatement: "Exchange with Profit, Return and Commission Excluded Test")
                .WithProfit(
                ...
                .WithCommission(
                    ...
                    isIncluded: false)
                .WithReturn(
                    returnWalletId: anisCardUsdWalletId.ToString(),
                    totalAmount: returnAmount,
                    description: $"Return {returnAmount} USD",
                    amount: returnAmount)
                .Build();
            ...
            Output.WriteLine($"   🔄 Return (USD): {returnAmount:F2}");
            ...
            var returnInfo = details.Return;
            if (returnInfo != null)
            {
                Output.WriteLine("\n🔄 معلومات الإرجاع (Return):");
                Output.WriteLine($"   💰 Return Wallet:          {returnInfo.Wallet?.WalletId}");
                Output.WriteLine($"   💰 Total Amount:           {returnInfo.TotalAmount:F4}");
                if (returnInfo.ReturnElements != null && returnInfo.ReturnElements.Count > 0)
                {
                    foreach (var element in returnInfo.ReturnElements)
                    {
                        Output.WriteLine($"   📝 {element.Description}: {element.Amount:F4}");
                    }
                }
            }
            ...
            var actualReturnAmount = returnInfo?.TotalAmount ?? 0m;
            ...
            Output.WriteLine($"   🔄 قيمة الإرجاع: {actualReturnAmount:F4}");
            ...
            var (isBuyFinalEqual, _, buyFinalMsg) = DecimalComparer.CompareBalance(expectedBuyAmount, actualBuyFinal, "مبلغ الشراء النهائي");
            ...
            // ✅ التحقق 12: قيمة الإرجاع (Return Amount)
            var (isReturnEqual, _, returnMsg) = DecimalComparer.CompareBalance(returnAmount, actualReturnAmount, "قيمة الإرجاع");
            Assert.True(isReturnEqual, returnMsg);
            Output.WriteLine($"   ✅ {returnMsg}");
            ...
            // ✅ التحقق 13: AnisPay LYD (زاد بمقدار sellLyd)
            ...
            // ✅ التحقق 14: AnisCard LYD (نقص بمقدار actualSellFinal)
     ...
     "AnisCard USD");
            ...
            // ✅ التحقق 17: Hreysh USD (نقص بمقدار expectedBuyAmount + returnAmount)
            var (isHreyshEqual, _, hreyshMsg) = DecimalComparer.CompareBalance(
                hreyshOld - (buyUsd + returnAmount),
                hreyshNew,
                "Hreysh USD");
            Assert.True(isHreyshEqual, hreyshMsg);
            Output.WriteLine($"   ✅ {hreyshMsg}");

            // ✅ التحقق 18: Commission Wallet (نقص بمقدار commissionAmount)
            ...
            var expectedNewAverageRate = ExchangeCalculator.CalcNewAvgRate(
                oldBalance,
                oldEstimatedLyd,
                expectedBuyAmount + returnAmount,  // buyUsd + commissionAmount + returnAmount
            ...
            // ✅ التحقق 19: متوسط سعر الصرف الجديد
            ...
            Output.WriteLine($"📊 ملخص الصرف مع الربح والعمولة غير المضمنة والإرجاع:");
            Output.WriteLine($"   💵 مبلغ الشراء (USD): {buyUsd:F2}");
            Output.WriteLine($"   💳 قيمة العمولة (USD): {commissionAmount:F2} (غير مضمنة)");
            Output.WriteLine($"   🔄 مبلغ الإرجاع (USD): {returnAmount:F2}");
            Output.WriteLine($"   📈 نسبة الربح: {profitRatio * 100}%");
            ...
            PrintResult(nameof(ExchangeWithProfitReturnCommissionExcluded_ShouldSucceed), true);
        }
    }
}